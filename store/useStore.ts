...
import { Patient, Consultation, TenantDetails, UserRegistration } from '../types';
...
export const useStore = create<AppState>((set, get) => ({
  ...
  register: async (userData) => {
    try {
      ...
      await tokenStorage.setDoctorData(response.doctor);
      await tokenStorage.setProfileComplete(false);
  ...
  login: async (email, password) => {
    try {
      ...
      // Check if profile is complete by looking at doctor data
      const hasClinicInfo = Boolean(
        response.doctor.clinic_name && 
        response.doctor.medical_registration_number
      );
      
      await tokenStorage.setProfileComplete(hasClinicInfo);

      // Load tenant details from storage if they exist
      const tenantData = hasClinicInfo ? await tokenStorage.getTenantDetails() : null;
  ...
  initializeSession: async () => {
    ...
    try {
      const [accessToken, refreshToken, doctorData, tenantData, profileComplete] = await Promise.all([
        tokenStorage.getAccessToken(),
        tokenStorage.getRefreshToken(),
        tokenStorage.getDoctorData(),
        tokenStorage.getTenantDetails(),
        tokenStorage.getProfileComplete(),
      ]);
  ...
  setTenantDetails: async (details) => {
    try {
      ...
      // Save to storage
      await tokenStorage.setDoctorData(updatedDoctor);
      await tokenStorage.setTenantDetails(details);
      await tokenStorage.setProfileComplete(true);
